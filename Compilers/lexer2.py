...
class SymbolTable:
    ...
    def push(self, name, line_numbers, type, scope):
        new_node = SymbolNode(name, line_numbers, type, scope)
    ...
    def checkforiden(self):
        if (self.temp != ''):
            ...
            if (self.temp in i):
                ...
                self.sym.push(self.temp, self.count,
            ...
            if blank_var[i] == '#':
                ...
                if (self.temp in integers):
                    ...
                    if (blank_var[i + 1] != '.' and blank_var[i+1] not in re.findall('[0-9]', blank_var)):
                        ...
                        j = re.findall(
                            '[a-zA-Z_][a-zA-Z_0-9]*', blank_var[i + 1])
                ...
                if (blank_var[i] == '\''):
                    ...
                    if (blank_var[i] == '\''):
                        ...
                        if (blank_var[i:i + 2] == '\\n' or blank_var[i:i + 2 == '\\t']):
                            if (blank_var[i + 2] == '\''):
                                self.tokens.append(
                                    Token(7, blank_var[i:i + 2], self.count))
                                self.temp = ''
                                i = i + 2
                                # self.tokens.append(Token(3, blank_var[i], self.count))
                                i = i + 1
                            ...
                            if (blank_var[i + 1] == '\''):
                                self.tokens.append(
                                    Token(7, blank_var[i], self.count))
                                self.temp = ''
                                i = i + 1
                                # self.tokens.append(Token(3, blank_var[i], self.count))
                                # i = i + 1
...
class Parser:
    ...
    def data(self):
        ...
        data = self.lookahead.data['value']
        conslist = ['INTEGER_CONSTANT', 'STRING_CONSTANT',
                    'FLOAT_CONSTANTS', 'CHAR_CONSTANTS', 'BOOL_CONST']
    ...
    def match(self, t):
        if (self.lookahead.data['value'] == t):
            ...
            print("ERROR WITH SYNATX NEAR TOKKEN: ",
                  self.lookahead.data['value'], " IN LINE NUMBER: ", self.lookahead.data['LINE_NUMBERS'])
            exit()
